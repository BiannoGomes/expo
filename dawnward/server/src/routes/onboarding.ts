...
export function registerOnboardingRoutes(app: FastifyInstance) {
  ...
  app.post("/onboarding/message", { preHandler: requireAuth }, async (request, reply) => {
    const userId = request.userId;
    const body = z.object({ text: z.string().min(1).max(8000) }).parse(request.body);

    // Consent gate (spec 04 §3): reflections are Article-9 territory —
    // nothing is processed until consent is explicitly on record.
    const [consentRow] = await query<{ granted: boolean | null }>(
      `select (consent->'reflections'->>'granted')::boolean as granted
