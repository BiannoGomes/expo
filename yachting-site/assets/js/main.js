...
(function () {
  ...
  /* ---- Graceful image fallback (photos not yet loaded) ---- */
  document.querySelectorAll('.gitem img').forEach(function (img) {
    img.addEventListener('error', function () {
      var fig = img.closest('.gitem');
      if (fig) fig.classList.add('is-missing');
    });
  });
  document.querySelectorAll('.about__portrait img').forEach(function (img) {
  ...
  items.forEach(function (f, i) {
    f.addEventListener('click', function () {
      if (f.classList.contains('is-missing')) return; // no photo to open yet
      open(i);
    });
  });
